...
| **🧠 Agent 记忆增强 (v5.3.7)** | 记忆重要度分析（重要度漂移+低估/高估识别+动态重评估建议，参考 Mem0）、上下文记忆注入（token 预算感知+格式化上下文字符串，参考 Letta）、Agent 情感追踪（情感时间线+转换序列+波动性评分，参考 Zep） |
| **🎬 AI 短剧增强 (v5.3.7)** | 类型趋势分析（上升/下降/稳定方向+平均评分+热门类型，竞品爆款风向标）、追剧粘性评分（多因子加权：节奏25%+张力25%+互动20%+经典15%+完成率15%）、角色关系深度分析（ally/rival/romance/family/mentor/stranger 六型分类） |
| **🔐 安全修复 (v5.3.7)** | _row_to_entry 防御性 JSON 解析（tags 已为 list 时不崩溃）、fuzzy_search LIKE 注入加固验证 |
| **🧠 Agent 记忆增强 (v5.3.6)** | 记忆关联推理（关键词重叠+标签共享+时间邻近度三维评分，自动构建记忆关联网络）、智能记忆召回（多因子综合评分：匹配覆盖率+重要度+访问频次+时间衰减+置顶加成） |
| **🎬 AI 短剧增强 (v5.3.6)** | 剧集节奏分析（滑动窗口密度+快/中/慢三档分类+拖沓/密集段识别+健康度评分）、角色互动分析（共现+台词交替+冲突词三维建模，互动矩阵+核心角色识别） |
| **🔐 安全加固 (v5.3.6)** | 全部 4 个新方法使用参数化 SQL + Unicode 控制字符过滤 + 长度/数量上限 + `_limited_fetch` 行数硬上限 |
...
**🧠 Agent 记忆增强**
- `memory-link` - 记忆关联推理（关键词重叠 Jaccard + 标签共享 + 时间邻近度三维加权评分，自动发现记忆间隐式关联，构建记忆关联网络，支持关联类型判定：keyword/tag/temporal/weak）
- `memory-recall` - 智能记忆召回（查询关键词提取 + 多因子综合评分：匹配覆盖率 40% + 重要度加权 20% + 访问频次 15% + 时间衰减 20% + 置顶加成 1.1x，Top-K 召回记忆）

**🎬 AI 短剧增强**
- `drama-pacing` - 剧集节奏分析（滑动窗口平均密度 + 快/中/慢三档分类 + 拖沓段/密集段识别 + 节奏健康度评分 + ASCII 节奏曲线可视化）
- `char-interaction` - 角色互动分析（场景共现 + 台词交替 + 冲突词三维建模，互动矩阵构建，关系类型识别：antagonist/close/frequent/casual，核心角色 Top-3 识别）

**🔐 安全加固**
- 全部 4 个新方法使用参数化 SQL（防 SQL 注入）
- 全部新方法 Unicode 控制字符过滤 + 长度上限（agent_id 128、drama_id 64、query 500、memory_id 64）
- `memory_link` / `memory_recall` 使用 `_limited_fetch` 行数硬上限（5000/10000）防全表扫描 DoS
- 修复 `char_interaction` 中遗留的无效代码块
