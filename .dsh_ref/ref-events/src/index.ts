// ref-events — a lifecycle/event plugin (generated by create-dsh-plugin).
//
...
export function apply(ctx: Context) {
  ...
  ctx.on('session/event', (session: Session, event: SessionEvent) => {
    ...
    if (sessionEvents <= 5 || sessionEvents % 25 === 0) {
      console.log(
        `[ref-events] session/event #${sessionEvents} type=${event.type} session=${String(session.id)}`,
      )
    }
  })
  ...
  ctx.on('tools/change', () => {
    toolChanges += 1
    console.log(`[ref-events] tools/change #${toolChanges}`)
  })
  ...
  ctx.on('tools/pre-execute', (exec: ToolExecution, next: () => Promise<PreToolDecision>) => {
    toolPreExecutes += 1
    console.log(`[ref-events] tools/pre-execute #${toolPreExecutes} tool=${exec.name}`)
    return next()
  })
  ...
  ctx.effect(() => {
    const timer = setInterval(() => {
      console.log(
        `[ref-events] heartbeat sessionEvents=${sessionEvents} toolPreExecutes=${toolPreExecutes} toolChanges=${toolChanges}`,
      )
    }, 30_000)
    return () => {
      clearInterval(timer)
      console.log(`[ref-events] DISPOSED — listeners removed, timer cleared`)
    }
  })

  console.log(`[ref-events] listeners registered: session/event + tools/change + tools/pre-execute`)
}
