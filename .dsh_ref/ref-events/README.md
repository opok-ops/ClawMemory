...
dsh plugin --profile my-profile add ./ref-events
dsh --profile my-profile       # watch for: [ref-events] listeners registered + tools/change #N
```
...
#   [ref-events] DISPOSED — listeners removed, timer cleared
```

## Dependencies pinned / 依赖锁定

- **runtime dependencies: none** — an event-only plugin imports nothing at runtime.
- `@deepseek-ai/dsh-tools` / `@deepseek-ai/dsh-session` are **devDependencies** (types only, `0.1.0-rc.6` / `0.1.0-rc.6` next-tag line).
