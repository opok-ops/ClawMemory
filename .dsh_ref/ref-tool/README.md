...
Registers the `ref_tool` tool via `defineTool()`.
...
dsh plugin --profile my-profile add ./ref-tool
dsh --profile my-profile       # watch for: [ref-tool] registered "ref_tool" — listed=true
```
...
```sh
dsh --profile my-profile --dump-config | grep ref-tool   # 配置层含本行
dsh plugin --profile headless add ./ref-tool
