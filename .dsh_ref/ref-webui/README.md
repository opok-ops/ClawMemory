...
Registers the `ref_webui` tool and renders its call/result as a **diff card** via
...
dsh plugin --profile my-profile add ./ref-webui
dsh --profile my-profile       # watch for: [ref-webui] registered "ref_webui" (webui diff card)
```
