...
Connects to real ClawMemory backend. Run: python app_native.py
"""
...
def add_memory(content, category, tags, privacy, importance):
    ...
    c.execute("INSERT INTO memories (id,content,plaintext_preview,category,tags,privacy,importance,"
        ...
        PM.get(privacy,1), importance, "gui","gui", now,now,now)
...
def get_memories(category="all", privacy="all", search=""):
    ...
    return [{"id":r[0],"content":r[1],"category":r[2],"tags":json.loads(r[3]),
             "privacy":PR.get(r[4],"INTERNAL"),"importance":r[5],
...
def get_memory_by_id(mid):
    ...
    return {"id":row[0],"content":row[1],"category":row[2],"tags":json.loads(row[3]),
             "privacy":PR.get(row[4],"INTERNAL"),"importance":row[5],
...
def update_memory(mid, content, category, tags, privacy, importance):
    now = time.time(); conn = sqlite3.connect(str(DB_PATH)); c = conn.cursor()
    c.execute("UPDATE memories SET content=?,category=?,tags=?,privacy=?,importance=?,updated_at=?,plaintext_preview=? WHERE id=?",
        content, category, json.dumps(tags, ensure_ascii=False), PM.get(privacy,1), importance, now, content[:200], mid)
...
def get_stats():
    ...
    db_size = DB_PATH.stat().st_size if DB_PATH.exists() else 0; conn.close()
...
def main():
        ...
        if event.startswith("card:"):
            ...
            if mem:
                ...
                ew = sg.Window("✏️编辑记忆 - ClawMemory",[
                    ...
                    [sg.Text("分类", font=("Segoe UI",9,"bold"), text_color="#64748b")],
                    [sg.Combo(list(CAT_LABELS.keys()), default_value=mem["category"], key="edit_cat", size=(20,1), readonly=True, background_color="#0a1020", text_color="#e2e8f0")],
                    [sg.Text("标签（逗号分隔）", font=("Segoe UI",9,"bold"), text_color="#64748b")],
    ...
    window.close()
    print("ClawMemory GUI 关闭。记忆数据保存在:", DB_PATH)
