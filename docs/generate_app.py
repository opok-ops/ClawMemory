#!/usr/bin/env python3
"""Generate complete app_native.py"""
output = []
...
# Privacy Scanner
PASSWORD_RE = re.compile(r'\u5bc6\u7801|\u53e3\u4ee4|\u8d26\u53f7|\u5bc6\u7387|\u9a8c\u8bc1|\u4fee\u6539|\u5bc6\u7801', re.I)
ID_RE = re.compile(r'\u8eab\u4efd\u8bc1|\u6237\u53e3|\u62a4\u7167|\u75c5\u5386|\u8eab\u4efd\u8bc1', re.I)
...
def seed_demo():
    ...
    demos = [
        ...
        ("AI Agent \u7ec8\u8eab\u8bb0\u5fc6\u662f\u8fd1\u4e24\u5e74\u6700\u6709\u4ef7\u503c\u7684\u521b\u4e1a\u65b9\u5411\u3002\u6838\u5fc3\u5783\u6b89\uff1a\u6570\u636e\u98de\u8f6e\u6548\u5e94\u3001\u9690\u79c1\u8ba1\u7b97\u57fa\u7840\u8bbe\u65bd\u3001Agent \u539f\u751f\u67b6\u6784\u3002","idea",["startup","ai","memory"],"PUBLIC",2),
...
def cat_accent(c): return {"learning":"#f59e0b","work":"#00d4ff","life":"#f472b6","idea":"#a855f7","fact":"#10b981","emotion":"#ef4444","general":"#64748b"}.get(c,"#00d4ff")

def build_cards(mems):
    if not mems: return [[sg.Text("\U0001f50d \u6682\u65e0\u8bb0\u5fc6",text_color="#334155",font=("Segoe UI",11),justification="center",pad=(20,40))]]
    cards = []
    for mem in mems:
        acc=cat_accent(mem["category"])
        priv_icon=PRIV_ICONS.get(mem["privacy"],"\U0001f7e1")
        priv_lbl=mem["privacy"]
        ...
        row = [[
            sg.Column([[
                sg.Text(CAT_ICONS.get(mem["category"],"\U0001f4cc")+" "+CAT_LABELS.get(mem["category"],mem["category"]),font=("Segoe UI",9,"bold"),text_color=acc,pad=(8,(8,0))),
                sg.Text(priv_icon+" "+priv_lbl,font=("Segoe UI",8),text_color="#64748b",pad=(0,(8,0)),expand_x=True,justification="right"),
            ...
            ]],background_color="#111a2e",pad=(0,4),expand_x=True,key="card:"+mid,metadata={"mem_id":mid}),
        ]]
        cards.append(row)
    return cards
...
def refresh(window, cat="all", search=""):
    ...
    window["status_text"].update("\u663e\u793a "+str(len(mems))+" \u6761 "+CAT_LABELS.get(cat,cat)+" \u8bb0\u5fc6")

def make_stats_win():
    s=get_stats()
    sz=s["db_size"]; sz_str=f"{sz/1024:.1f} KB" if sz<1048576 else f"{sz/1048576:.1f} MB"
    pcols={"PUBLIC":"#10b981","INTERNAL":"#f59e0b","PRIVATE":"#ef4444","STRICT":"#a855f7"}
    prow=[sg.Text(PRIV_ICONS.get(k,"\U0001f7e0")+" "+k+": "+str(v)+" \u6761",text_color=pcols.get(k,"#94a3b8"),font=("Segoe UI",10,"bold")) for k,v in s.get("by_privacy",{}).items()]
    crow=[sg.Text("  "+CAT_ICONS.get(k,"\U0001f4cc")+" "+CAT_LABELS.get(k,k)+": "+str(v)+" \u6761",text_color="#94a3b8") for k,v in list(s.get("top_categories",{}).items())[:10]]
    return sg.Window("\U0001f4ca\u7edf\u8ba1\u62a5\u544a - ClawMemory",[
        ...
        [sg.Text("\u2014 \u9690\u79c1\u5206\u7ea7\u2014",font=("Segoe UI",9,"bold"),text_color="#334155",pad=(0,4))],*prow,
        [sg.Text("\u2014 \u5206\u7c7b\u2014",font=("Segoe UI",9,"bold"),text_color="#334155",pad=(0,4))],*crow,
...
def make_privacy_win():
    ...
    return sg.Window("\U0001f6e1\ufe0f\u9690\u79c1\u7ba1\u7406 - ClawMemory",[
...
def main():
    ...
    cat_btns={c:sg.Button(CAT_ICONS[c]+" "+CAT_LABELS[c],key="cat:"+c,font=("Segoe UI",9),
        button_color=("#1e293b","#00d4ff" if c=="all" else "#475569"),
        border_width=0,pad=(2,2),size=(8,1)) for c in cats}
    ...
    layout=[
        ...
        ]],background_color="#0d1422",pad=(16,12))],
        [sg.HorizontalSeparator(color="#1e3a5f")],
        ...
        ]],background_color="#0d1422",pad=(12,4),expand_x=True)],
    ...
    while True:
        ...
        if event=="btn_add":
            add_win=sg.Window("+\u6dfb\u52a0\u65b0\u8bb0\u5fc6 - ClawMemory",[
                ...
                [sg.Multiline(key="add_content",size=(60,7),background_color="#0a1020",text_color="#e2e8f0",border_width=1,focus=True,pad=(0,(0,4)))],
                [sg.Text("\u5206\u7c7b",font=("Segoe UI",9,"bold"),text_color="#64748b")],
                [sg.Combo(list(CAT_LABELS.keys()),default_value="general",key="add_cat",size=(20,1),readonly=True,background_color="#0a1020",text_color="#e2e8f0")],
                ...
                [sg.Combo(IMP_LABELS,default_value="\u4e2d",key="add_imp",size=(20,1),readonly=True,background_color="#0a1020",text_color="#e2e8f0")],
                [sg.Checkbox("\u3010\u63a8\u8350\u3011\u81ea\u52a8\u9690\u79c1\u68c0\u6d4b",key="auto_priv",default=True,text_color="#64748b",background_color="#0d1422",pad=(0,(8,0)))],
                [sg.Button("\u2705\u4fdd\u5b58\u8bb0\u5fc6",key="save_add",button_color=("#0d1422","#00d4ff"),font=("Segoe UI",10,"bold"),pad=(0,(16,0))),sg.Button("\u53d6\u6d88",key="cancel_add",button_color=("#1e293b","#475569"))],
            ],modal=True,finalize=True,background_color="#0d1422",size=(480,560))
            add_win["add_content"].set_focus()
            while True:
                ev2,v2=add_win.read()
                if ev2 in(sg.WINDOW_CLOSED,"cancel_add"): add_win.close(); break
                if ev2=="save_add":
                    content=v2.get("add_content","").strip()
                    if not content: add_win["add_content"].update(""); continue
                    cat=v2.get("add_cat","general")
                    tags_str=v2.get("add_tags","").strip()
                    tags=[t.strip() for t in tags_str.split(",") if t.strip()]
                    priv=v2.get("add_privacy","INTERNAL")
                    imp_val=v2.get("add_imp","\u4e2d")
                    imp_map={"\u4f4e":1,"\u4e2d":2,"\u9ad8":3,"\u5173\u952e":4}
                    imp=imp_map.get(imp_val,2)
                    if v2.get("auto_priv"):
